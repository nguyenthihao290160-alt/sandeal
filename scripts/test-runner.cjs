...
(async () => {
  ...
  const fixtureLinkCheckedAt = new Date().toISOString();
  ...
  await test('20. published xuất hiện public selector', async () => { const ready = indexableProduct({ id: 'public', sourceId: 'public' }); await adapter.writeCollection('products', [ready]); equal((await products.getPublicProducts()).length, 1); });
