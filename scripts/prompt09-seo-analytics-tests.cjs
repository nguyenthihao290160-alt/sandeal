...
function product(id, overrides = {}) {
  return {
    ...
    canonicalUrlSource: 'manual', canonicalUrlStatus: 'verified', canonicalUrlVerifiedAt: TEST_NOW,
    affiliateUrl: `https://merchant.example/products/${id}?affiliate=private-fixture`,
    affiliateUrlSource: 'manual', affiliateUrlStatus: 'verified', affiliateUrlVerifiedAt: TEST_NOW,
...
async function main() {
  ...
  function publicProduct(id, overrides = {}) {
    ...
    assert.equal(
      ...
      true,
      `fixture failed public filter: ${publicFilter.getPublicProductBlockReason(published)}`,
    );
    return published;
  }
