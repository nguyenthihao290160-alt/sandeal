...
export default function ProductsDashboard() {
  ...
  const runItemAction = async (action: 'approve' | 'archive', item: DashboardProductItem) => {
    ...
    try {
      ...
      if (!['waiting_approval', 'waiting_manual', 'completed', 'failed', 'cancelled', 'blocked'].includes(next.status)) {
        void pollOperation(body.data.jobId).catch((reason) => {
          if (mountedRef.current) setOperation((current) => current ? {
            ...current,
  ...
  const pollOperation = async (jobId: string) => {
    ...
    if (mountedRef.current && !controller.signal.aborted) setOperation((current) => current ? { ...current, message: 'Tác vụ vẫn đang xử lý. Việc theo dõi tự động đã dừng sau thời gian giới hạn; dữ liệu không bị thay đổi bởi thao tác theo dõi.' } : current);
  };
