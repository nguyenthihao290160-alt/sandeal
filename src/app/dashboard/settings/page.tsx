...
export default function SafetySettingsPage() {
  ...
  const isEmergencyAction = action === 'enable_kill_switch' || action === 'disable_kill_switch';
  return <main className={styles.page} aria-busy={loading}>
    ...
    {control && <div className={styles.grid}>
      <section className={`${styles.panel} ${styles.infoPanel}`}>
        <div className={styles.panelHeader}><h2><DashboardIcon name="settings" size={19} />Điều khiển vận hành</h2></div>
        <div className={styles.healthList}>
          <div className={styles.healthRow}><div><span className={styles.name}>Bộ xử lý nền</span><span className={styles.muted}>Không nhận tác vụ mới khi tạm dừng; tác vụ đang chạy được kết thúc an toàn.</span></div><button className={control.workerPaused ? styles.primary : styles.warningButton} onClick={() => { setAction(control.workerPaused ? 'resume_worker' : 'pause_worker'); setReason(''); }}>{control.workerPaused ? 'Tiếp tục' : 'Tạm dừng'}</button></div>
          <div className={styles.healthRow}><div><span className={styles.name}>Lịch chạy tự động</span><span className={styles.muted}>Không tạo tác vụ mới khi tạm dừng; hàng chờ hiện có được giữ nguyên.</span></div><button className={control.schedulerPaused ? styles.primary : styles.warningButton} onClick={() => { setAction(control.schedulerPaused ? 'resume_scheduler' : 'pause_scheduler'); setReason(''); }}>{control.schedulerPaused ? 'Tiếp tục' : 'Tạm dừng'}</button></div>
        </div>
        {action && !isEmergencyAction && <div className={styles.notice} role="region" aria-label="Xác nhận điều khiển vận hành"><strong>{ACTION_LABELS[action]}</strong><label htmlFor="inline-control-reason">Lý do<textarea id="inline-control-reason" value={reason} onChange={event => setReason(event.target.value)} rows={2} autoFocus /></label><span className={styles.muted}>Thay đổi được lưu bền vững; không xóa tác vụ hiện có.</span><div className={styles.actions}><button className={styles.button} onClick={() => setAction(null)} disabled={busy}>Hủy</button><button className={styles.primary} disabled={busy || reason.trim().length < 8} onClick={() => void submit()}>{busy ? 'Đang cập nhật' : 'Xác nhận inline'}</button></div></div>}
      </section>
    ...
    {action && isEmergencyAction && <div className={styles.dialogBackdrop} onMouseDown={event => { if (event.target === event.currentTarget && !busy) setAction(null); }}><section className={styles.dialog} role="alertdialog" aria-modal="true" aria-labelledby="control-title"><h2 id="control-title">{ACTION_LABELS[action]}</h2><p><strong>Việc sẽ dừng:</strong> lịch tạo tác vụ, worker nhận việc mới, lời gọi AI và đăng sản phẩm.</p><p><strong>Website công khai:</strong> vẫn tiếp tục phục vụ. Tác vụ đang chạy chỉ tiếp tục đến điểm kiểm tra an toàn tiếp theo.</p><p><strong>Người kích hoạt:</strong> tài khoản quản trị đang đăng nhập. Dừng khẩn cấp không tự tắt.</p><label htmlFor="control-reason">Lý do<textarea id="control-reason" value={reason} onChange={event => setReason(event.target.value)} rows={3} autoFocus aria-describedby="control-help" /></label><p id="control-help" className={styles.muted}>Nhập ít nhất 8 ký tự để xác nhận và lưu nhật ký.</p><div className={styles.dialogActions}><button className={styles.button} onClick={() => setAction(null)} disabled={busy}>Đóng</button><button className={styles.danger} disabled={busy || reason.trim().length < 8} onClick={() => void submit()}>{busy ? 'Đang cập nhật' : 'Xác nhận thay đổi'}</button></div></section></div>}
  </main>;
}
