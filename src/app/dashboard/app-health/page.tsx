  ...
  return <main className={styles.page} aria-busy={loading}>
    ...
    {health && <>
      ...
      <div className={styles.grid}>
        <section className={`${styles.panel} ${health.worker.status === 'active' ? styles.successPanel : health.worker.status === 'stale' ? styles.dangerPanel : styles.warningPanel}`}><div className={styles.panelHeader}><h2><DashboardIcon name="worker" size={19} />Bộ xử lý nền</h2><span className={stateClass(health.worker.status)}>{STATE[health.worker.status] || 'Không thể xác minh'}</span></div><div className={styles.healthList}><div className={styles.healthRow}><span>Tín hiệu gần nhất</span><strong>{when(health.worker.heartbeatAt)}</strong></div><div className={styles.healthRow}><span>Tuổi / nguồn heartbeat</span><strong>{health.worker.heartbeatAgeMs === null ? 'Chưa có' : `${Math.round(health.worker.heartbeatAgeMs / 1000)} giây`} · {health.worker.heartbeatSource}</strong></div><div className={styles.healthRow}><span>Release</span><strong>{health.worker.releaseId?.slice(0, 12) || 'Chưa ghi nhận'}</strong></div><div className={styles.healthRow}><span>Tác vụ hiện tại</span><strong>{health.worker.currentJobId || 'Không có'}</strong></div><div className={styles.healthRow}><span>Định danh bộ xử lý</span><strong>{health.worker.workerId || 'Chưa ghi nhận'}</strong></div></div>{health.worker.status === 'unverified' && <div className={styles.notice}><DashboardIcon name="warning" size={16} /> Chưa nhận được tín hiệu bộ xử lý. Hãy khởi động worker rồi thử lại.</div>}</section>

        <section className={`${styles.panel} ${health.scheduler.status === 'active' ? styles.successPanel : health.scheduler.status === 'stale' ? styles.dangerPanel : styles.warningPanel}`}><div className={styles.panelHeader}><h2><DashboardIcon name="scheduler" size={19} />Lịch chạy tự động</h2><span className={stateClass(health.scheduler.status)}>{STATE[health.scheduler.status] || 'Không thể xác minh'}</span></div><div className={styles.healthList}><div className={styles.healthRow}><span>Heartbeat hiện tại</span><strong>{when(health.scheduler.heartbeatAt)}</strong></div><div className={styles.healthRow}><span>Tuổi / nguồn heartbeat</span><strong>{health.scheduler.heartbeatAgeMs === null ? 'Chưa có' : `${Math.round(health.scheduler.heartbeatAgeMs / 1000)} giây`} · {health.scheduler.heartbeatSource}</strong></div><div className={styles.healthRow}><span>Release</span><strong>{health.scheduler.releaseId?.slice(0, 12) || 'Chưa ghi nhận'} / web {health.release.releaseId.slice(0, 12)}</strong></div><div className={styles.healthRow}><span>Lần chạy gần nhất</span><strong>{when(health.scheduler.lastRunAt)}</strong></div><div className={styles.healthRow}><span>Lần chạy tiếp theo</span><strong>{when(health.scheduler.nextRunAt)} · {health.scheduler.scheduleState}</strong></div><div className={styles.healthRow}><span>Múi giờ</span><strong>Việt Nam (UTC+7)</strong></div></div>{health.scheduler.status === 'unverified' && <div className={styles.notice}><DashboardIcon name="warning" size={16} /> Chưa nhận được tín hiệu lịch tự động. Hãy khởi động scheduler rồi thử lại.</div>}</section>
