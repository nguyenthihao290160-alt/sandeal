...
const TYPES = new Set<AutomationJobType>(['PRODUCT_SCAN','AUTO_PILOT','SAFE_PUBLISH','AI_ANALYSIS','HEALTH_CHECK']);
...
export async function POST(request: NextRequest) {
  ...
  const riskLevel = dryRun || type === 'HEALTH_CHECK' ? 'LOW' : ['AUTO_PILOT','SAFE_PUBLISH','AI_ANALYSIS','PRODUCT_SCAN'].includes(type) ? 'HIGH' : 'MEDIUM';
  try {
    const result = await createAutomationJob({ type, payload: body.payload && typeof body.payload === 'object' && !Array.isArray(body.payload) ? body.payload as Record<string, unknown> : {},
      idempotencyKey, operationId: typeof body.operationId === 'string' ? body.operationId : undefined, requestedBy: 'dashboard-admin',
